...
function showScores() {
  ...
  const totalColorClass = total >= 0 ? 'positive-score' : 'negative-score';

  totalRow.innerHTML = `
    <td class="total-score"><strong>Total Score</strong></td>
    <td class="total-score ${totalColorClass}"><strong>${total >= 0 ? "+" : ""}${total.toFixed(0)}</strong></td>
    <td class="total-score ${totalColorClass}"><strong>${totalScoreComment}</strong></td>`;
  tbody.appendChild(totalRow);

  table.style.display = "table";
  if (totalBox && totalCommentEl) {
    totalCommentEl.textContent = summaryBelowTable;
    totalCommentEl.classList.remove("positive-comment", "negative-comment");
    totalCommentEl.classList.add(total >= 0 ? "positive-comment" : "negative-comment");
